...
public class CedulaMainActivity extends AutoMainActivity {
    ...
    void showCedulaSearch(){
        ...
        bc.setOnClickListener(v->{String x=curp.getText().toString().trim().toUpperCase();if(!validCurp(x)){toast("CURP inválida");return;}Intent i=new Intent(this,SepCedulaFlexibleRpaActivity.class);i.putExtra("mode","CURP");i.putExtra("curp",x);startActivityForResult(i,REQ_SEP_FLEX);});
    ...
    @Override void launchSepRpa(String curp){Intent i=new Intent(this,SepCedulaFlexibleRpaActivity.class);i.putExtra("mode","CURP");i.putExtra("curp",curp);startActivityForResult(i,REQ_SEP_RPA);}
    ...
    @Override void showPersonResult(String curp,String rfc,String status,String checked,boolean cacheHit,long qid){pendingCurp=curp;pendingRfc=rfc;pendingQueryId=qid;if(!hasSepRecord(curp)&&!sepAttempted){sepAttempted=true;base("Consultando cédulas…",true);LinearLayout wait=card();wait.addView(tv("CÉDULAS PROFESIONALES SEP",20,BLUE,true));wait.addView(tv("ConsultaMX está buscando automáticamente en el sistema oficial de la SEP usando la CURP. Sólo se aceptará como resultado una respuesta real, nunca la pantalla inicial.",14,NAVY,false));body.addView(wait);launchSepRpa(curp);return;}super.showPersonResult(curp,rfc,status,checked,cacheHit,qid);renderSepSummary(curp);}

    @Override protected void onActivityResult(int requestCode,int resultCode,Intent data){if(requestCode==REQ_SEP_FLEX){toast(resultCode==RESULT_OK?"Consulta SEP guardada":"La consulta SEP no terminó");showAllResponseRecords();return;}if(requestCode==REQ_SEP_RPA)sepAttempted=true;super.onActivityResult(requestCode,resultCode,data);}
